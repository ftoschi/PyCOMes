...
@jit(nopython=True, nogil=True, cache=True)
def diffuse(dn, E, diff_t, diff_l, drift, dt):
    ...
    if len(E) == 2:
        ...
        sigma_l = np.sqrt(
            2 * Dl * dt[0] * 1e-6) * 10  # diffusion constant is in cm²/s and time in µs -> 1e-6 to convert to seconds
