...
def closest_point_grid_3D(p, X, Y, Z):
        ...
        return_indeces = np.array([indeces[0]-1, indeces[0], indeces[1]-1, indeces[1], indeces[2]-1, indeces[2]], dtype = np.int32)
        return return_indeces
...
@jit(nopython = True, nogil = True, cache = True)
def interpolate_field_3D(p, X, Y, Z, field_components):
        ...
        Ex  =  field_components[0]
        Ey  =  field_components[1]
        Ez  =  field_components[2]

        i = xl+lenx*yl+lenx*leny*zl; LLL = np.array([Ex[i],Ey[i],Ez[i]], dtype = np.float64)
        i = xl+lenx*yl+lenx*leny*zu; LLU = np.array([Ex[i],Ey[i],Ez[i]], dtype = np.float64)
        i = xl+lenx*yu+lenx*leny*zl; LUL = np.array([Ex[i],Ey[i],Ez[i]], dtype = np.float64)
        i = xl+lenx*yu+lenx*leny*zu; LUU = np.array([Ex[i],Ey[i],Ez[i]], dtype = np.float64)
        i = xu+lenx*yl+lenx*leny*zl; ULL = np.array([Ex[i],Ey[i],Ez[i]], dtype = np.float64)
        i = xu+lenx*yl+lenx*leny*zu; ULU = np.array([Ex[i],Ey[i],Ez[i]], dtype = np.float64)
        i = xu+lenx*yu+lenx*leny*zl; UUL = np.array([Ex[i],Ey[i],Ez[i]], dtype = np.float64)
        i = xu+lenx*yu+lenx*leny*zu; UUU = np.array([Ex[i],Ey[i],Ez[i]], dtype = np.float64)
...
@jit(nopython = True, nogil = True, cache = True)
def trajectory_line(p, X, Y, field_components, dn, edges, diff_t=np.array([[],[]]), diff_l=np.array([[],[]]), drift=np.array([[],[]]), diffuse_on=False, print_point = False):
        ...
        while is_inside(p, edges):
                ...
                if diffuse_on:
                        dp = diffuse(dp, E, diff_t, diff_l, drift, dt)
                ...
                t_tmp = np.concatenate((t_tmp, np.array([time])))
                if print_point:
                        print(p, time)
...
@jit(nopython = True, nogil = True, cache = True)
def trajectory_line_3D(p, X, Y, Z, field_components, dn, edges, diffusion_on = False, print_point = False):
        ...
        while is_inside(p, edges):
                E = interpolate_field_3D(p, X, Y, Z, field_components)
                if np.isnan(E[0]):
                        break
                normE = np.linalg.norm(E)
                dp = -dn*E/normE
